...
public class YoutubeRequest {
    static let shared = YoutubeRequest()
    ...
    typealias YoutubePlaylistHTTPCompletion = (Result<YouTubeData, Error>) -> Void
    func getVideos(playlistId: String, url: String, tdAppId:String, completion: @escaping YoutubePlaylistHTTPCompletion) {
        DispatchQueue.global().async { [weak self] in
            guard let strongSelf = self else {return}
            let video = YouTubeData(duration:nil, url: "", title: "", dateYT: "", dateIA: "", timeYT: "", timeIA: "", description: "", thumbnailURL: "", channelTitle: "", viewsYT: nil, viewsIA: 0, toneDeafAppId: "", youtubeId: "", type: "YTPL", isActive: false)
            let resourseString = "https://www.googleapis.com/youtube/v3/playlists?part=snippet&part=contentDetails&part=status&id=\(playlistId)&key=\(strongSelf.apiKey)"
            guard let resourceURL = URL(string: resourseString) else {fatalError()}
            AF.request(resourceURL, method: .get, parameters: ["part":"snippet", "key":strongSelf.apiKey], encoding: URLEncoding.default, headers: nil).responseJSON { response in
                if let status = response.response?.statusCode {
                    switch(status){
...
public enum YoutubeRequestErrors: Error {
    case status400
}

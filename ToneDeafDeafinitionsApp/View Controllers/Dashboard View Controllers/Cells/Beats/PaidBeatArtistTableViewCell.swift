//
//  PaidBeatArtistTableViewCell.swift
...
class PaidBeatArtistTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var leaseStack: UIStackView!
    @IBOutlet weak var leasePrice: UILabel!
    @IBOutlet weak var leaseLabel: UILabel!
    @IBOutlet weak var leaseCart: UIImageView!
    ...
    override func prepareForReuse() {
        ...
        exclusiveCart.image = UIImage(systemName: "cart.badge.plus")
        exclusiveCart.tintColor = Constants.Colors.redApp
        
        wavCart.image = UIImage(systemName: "cart.badge.plus")
        wavCart.tintColor = Constants.Colors.redApp
        wavPrice.text = "N/A"
        wavPrice.textColor = .white
        wavPrice.alpha = 0.6
        wavLabel.alpha = 0.6
        
        leaseCart.image = UIImage(systemName: "cart.badge.plus")
        leaseCart.tintColor = Constants.Colors.redApp
        leasePrice.text = "N/A"
        leasePrice.textColor = .white
        leasePrice.alpha = 0.6
        leaseLabel.alpha = 0.6
    }
    ...
    func setBeat(beat: BeatData) {
        ...
        if let price = beat.mp3Price {
            
            leasePrice.text = price.dollarString
            leasePrice.textColor = .white
            leasePrice.alpha = 1
            leaseLabel.alpha = 1
        }
        for index in 0..<userCart.count {
            ...
            if item.id.contains(beat.toneDeafAppId) && item.id.contains("mp3") {
                leaseCart.image = UIImage(systemName: "cart.fill")
                leaseCart.tintColor = .green
                leasePrice.text = "In Cart"
                mp3InCart = true
            }
            if item.id.contains(beat.toneDeafAppId) && item.id.contains("exclusive") {
                exclusiveCart.image = UIImage(systemName: "cart.fill")
                exclusiveCart.tintColor = .green
                exclusivePrice.text = "In Cart"
                exclusiveInCart = true
            }
            if item.id.contains(beat.toneDeafAppId) && item.id.contains("wav") {
                wavCart.image = UIImage(systemName: "cart.fill")
                wavCart.tintColor = .green
                wavPrice.text = "In Cart"
                wavInCart = true
            }
        }
        var proarray:[String] = []
    ...
    func setGestures() {
        ...
        let mp3gesture = UITapGestureRecognizer(target: self, action: #selector(mp3Tapped(_:)))
        ...
        mp3gesture.numberOfTapsRequired = 1
        leaseStack.addGestureRecognizer(mp3gesture)
    ...
    @objc func downloadTapped(_ sender: UITapGestureRecognizer? = nil) {
        print("tapppppppp d load")
    }
    
    @objc func mp3Tapped(_ sender: UITapGestureRecognizer? = nil) {
        guard let mp3price = currentBeat.mp3Price else {return}
        switch mp3InCart {
        case true:
            for index in 0..<userCart.count {
                let item = userCart[index].product
                if item.id == "\(currentBeat.toneDeafAppId)Æ\(currentBeat.name)Æmp3" {
                    let alerticon = UIImage(named: "mp3-file")!.withTintColor(.white)
                    let actionSheet = CDAlertView(title: "Remove \"\(currentBeat.name)\" Mp3 Lease License from Cart?", message: "Mp3 Lease License", type: .custom(image: alerticon))
                    ...
                    actionSheet.add(action: CDAlertViewAction(title: "Remove", font: UIFont.boldSystemFont(ofSize: 16), textColor: Constants.Colors.redApp, backgroundColor: Constants.Colors.mediumApp, handler: {[weak self]_ in
                        ...
                        strongSelf.mp3InCart = false
                        strongSelf.leaseCart.image = UIImage(systemName: "cart.badge.plus")
                        strongSelf.leaseCart.tintColor = Constants.Colors.redApp
                        if let price = strongSelf.currentBeat.mp3Price {
                            
                            strongSelf.leasePrice.text = price.dollarString
                        }
                        strongSelf.leasePrice.textColor = .white
                        strongSelf.leasePrice.alpha = 1
                        strongSelf.leaseLabel.alpha = 1
                        return true
                    }))
        ...
        default:
            let product = Product(id: "\(currentBeat.toneDeafAppId)Æ\(currentBeat.name)Æmp3", dbid: "\(currentBeat.toneDeafAppId)Æ\(currentBeat.name)", name: currentBeat.name, price: 0.0, thumbnailURL: currentBeat.imageURL, type: "Mp3", involved: currentBeat.producers)
            let alerticon = UIImage(named: "mp3-file")!.withTintColor(.white)
            let actionSheet = CDAlertView(title: "Add \"\(currentBeat.name)\" Mp3 Lease License to Cart?", message: "Mp3 Lease License", type: .custom(image: alerticon))
            actionSheet.alertBackgroundColor = Constants.Colors.mediumApp
            actionSheet.circleFillColor = .black
            actionSheet.titleTextColor = .white
            actionSheet.messageTextColor = .white
            ...
            actionSheet.add(action: CDAlertViewAction(title: "Add", font: UIFont.boldSystemFont(ofSize: 16), textColor: Constants.Colors.redApp, backgroundColor: Constants.Colors.mediumApp, handler: {[weak self]_ in
                guard let strongSelf = self else {return false}
                product.price = mp3price
                userCart.add(product)
                strongSelf.leaseCart.image = UIImage(systemName: "cart.fill")
                strongSelf.leaseCart.tintColor = .green
                strongSelf.leasePrice.text = "In Cart"
                strongSelf.mp3InCart = true
                return true
            }))
            NotificationCenter.default.post(name: OpenTheAlertNotify, object: actionSheet)
            return

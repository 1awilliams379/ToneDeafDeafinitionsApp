...
public class YoutubeRequest2 {
    static let shared = YoutubeRequest2()
            ...
            AF.request(resourceURL, method: .get, parameters: ["part":"snippet", "key":strongSelf.apiKey], encoding: URLEncoding.default, headers: nil).responseJSON { response in
                if let status = response.response?.statusCode {
                    ...
                    case 200:
                        ...
                        print("example success", response.data)
                    default:
                        print("error with response status: \(status)")
                        completion(.failure(AlbumUploadTextFieldErrors.invalidURL))
                        return
                    }
                }
                else {
